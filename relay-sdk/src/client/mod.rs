...
pub enum Notify {
    ...
    /// Route state changed.
    RouteChanged { has_relay_route: bool, fallback_to_direct: bool, flags: u32 },
...
impl ClientInner {
    ...
    fn handle_command(&mut self, cmd: Command) {
        match cmd {
            ...
            Command::OpenSession { server_address, client_secret_key } => {
            ...
            Command::RouteUpdate { update_type, num_tokens, tokens, magic, client_external_address } => {
                if self.session_open {
                    self.magic                   = magic;
                    self.client_external_address = client_external_address.clone();
                    self.route_manager.update(
                        update_type, num_tokens, &tokens,
                        &self.client_secret_key, &magic, &client_external_address,
                    );
    ...
    pub fn open_session(&mut self, server_address: Address, client_secret_key: [u8; XCHACHA_KEY_BYTES]) {
    ...
    pub fn process_incoming(&mut self, data: &[u8]) -> Option<Vec<u8>> {
        ...
        match data[0] {
            ...
            PACKET_TYPE_SERVER_TO_CLIENT => {
                if let Some(seq) = self.route_manager.process_server_to_client_packet(
                    PACKET_TYPE_SERVER_TO_CLIENT, data,
                ) {
                    // Replay check.
                    if self.payload_replay.already_received(seq) { return None; }
                    let body_off = PACKET_BODY_OFFSET + HEADER_BYTES;
                    if data.len() <= body_off { return None; }
    ...
    fn try_send_pending(&mut self) {
        ...
        if let Some((to, len)) = self.route_manager.send_route_request(self.send_buf.as_mut()) {
            let data = self.send_buf[..len].to_vec();
            self.push_notify(Notify::SendRaw { to, data });
        }
        // Try to send a pending continue request.
        if let Some((to, len)) = self.route_manager.send_continue_request(self.send_buf.as_mut()) {
            let data = self.send_buf[..len].to_vec();
            self.push_notify(Notify::SendRaw { to, data });
        }
    }
...
impl Client {
    ...
    pub fn open_session(&mut self, server_address: Address, client_secret_key: [u8; XCHACHA_KEY_BYTES]) {
        ...
        self.push_command(Command::OpenSession { server_address, client_secret_key });
    ...
    pub fn route_update(
        ...
        self.push_command(Command::RouteUpdate {
            update_type, num_tokens, tokens, magic, client_external_address,
        });
    }
    ...
    pub fn send_packet(&mut self, payload: &[u8]) {
        if self.state != CLIENT_STATE_OPEN { return; }
        self.push_command(Command::SendPacket { payload: payload.to_vec() });
    }
    ...
    fn apply_notify(&mut self, n: Notify) {
        match n {
            Notify::RouteChanged { has_relay_route, fallback_to_direct, flags } => {
    ...
    pub fn is_session_open(&self)     -> bool { self.state == CLIENT_STATE_OPEN }
    pub fn has_relay_route(&self)     -> bool { self.has_relay_route }
    pub fn is_fallback_direct(&self)  -> bool { self.fallback_to_direct }
...
mod tests {
    ...
    fn key() -> [u8; XCHACHA_KEY_BYTES] { [0xABu8; XCHACHA_KEY_BYTES] }
    ...
    fn client_send_packet_no_relay_route_yields_no_send_raw() {
        ...
        let has_send_raw = inner.notify.lock().unwrap().iter().any(|n| matches!(n, Notify::SendRaw { .. }));
        assert!(!has_send_raw, "expected no SendRaw without relay route, notify_count={notify_count}");
    }
    ...
    fn client_route_update_direct_type_does_not_fallback() {
        let (mut inner, mut client) = make_pair();
        let ext = Address::V4 { octets: [1, 2, 3, 4], port: 5555 };
        client.open_session(addr(), key());
        inner.pump_commands();
