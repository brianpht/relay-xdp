...
5. ~~**Medium - P6:** Add `build-ebpf-rust-profiling` command in `xtask/src/main.rs`. Deploy profiling build to staging relay, read `/metrics` counters, compute avg ns per stage vs targets in `docs/PERFORMANCE_DESIGN.md`.~~
   **DONE 2026-05-27** - `build-ebpf-rust-profiling` xtask already existed. Fixed aya API path (`helpers::` not `bindings::`). Added Ansible playbook `ebpf-profiling-deploy.yml` + `ebpf-profiling-restore.yml` and Makefile targets `ebpf-profiling-deploy` / `ebpf-profiling-restore`. Fixed measurement bug: parse_ns/filter_ns were recorded before early-return ping dispatch, inflating per-sample avg ~2x; deferred to full-path tail. Added `CRYPTO_NS` counter (t5 before dispatch -> t_end after handler). All 5 relays deployed. Results: parse ~40 ns (PASS), filter ~45 ns (EC2 overhead), map_lookup ~165 ns (over 100 ns budget, EC2 L3 latency), crypto+rewrite ~190 ns (PASS < 550 ns). Total 547-760 ns (PASS < 1 us). See BENCH_RESULTS.md Step 12.
...
| Modified | `relay-xdp-ebpf/src/main.rs` - profiling fix: deferred parse/filter recording to full-path tail; added CRYPTO_NS counter (t5->t_end); fixed aya helpers:: path |
| Added | `ansible/playbooks/ebpf-profiling-deploy.yml` - deploy profiling eBPF binary to relay nodes with backup/restore, serial roll, service verify |
| Added | `ansible/playbooks/ebpf-profiling-restore.yml` - restore production binary from backup after profiling session |
| Modified | `Makefile` - added `ebpf-profiling-deploy` and `ebpf-profiling-restore` targets with full documentation |
| Modified | `docs/BENCH_RESULTS.md` - added Step 12 (P6 profiling results), updated ToC, updated P6 improvement area to RESOLVED |
