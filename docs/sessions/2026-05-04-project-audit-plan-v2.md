...
| relay-sdk (FFI / crypto)         | 2 (-1)      | 3          | 3 (+1)    | 2         |
| Infra / Ansible / CI             | 3           | 2          | 4         | 1 (-1)    |
| **Total**                        | **11** (-3) | **17** (+2)| **19**    | **13** (=)|

Net change after v2 verification:
...
- High 15 -> 17 (lock-poisoning sites now sit here explicitly; backend HTTP route auth surface NEW after Pulumi SG check confirmed wide-open ingress).
...
|-----------|----------------|-------|---------------|-------|
| P1-01 | Critical theme #1 (replay) | `relay-backend/src/handlers.rs`, `state.rs`, `relay_update.rs`, `metrics.rs` | New `tests/e2e_encrypted.rs` cases: replay rejected; stale `current_time` rejected; window expiry replay accepted | Per-relay LRU 1024 nonces; `30s` clock-skew window. Companion [ADR-004](../decisions/ADR-004-replay-protection-window-for-relay-update.md) |
| P1-02 | Critical theme #2 (FFI zeroize + panic policy) | `relay-sdk/src/ffi/mod.rs`, new `ffi/panic.rs`, possibly `client.rs`, `server.rs` | Compile-time `static_assertions::assert_impl_all!(Zeroizing<[u8; 32]>: Zeroize)`; new test that registers a panic hook and asserts it fires | `zeroize = "1"` already vendored at `relay-sdk/Cargo.toml:27`. Panic-hook surface from companion [ADR-006](../decisions/ADR-006-ffi-panic-policy-for-relay-sdk.md) |
...
| P1-04 | Critical theme #4 (Pulumi prod CIDR) | `infra/Pulumi.production.yaml`, `infra/config.py`, `Makefile` | Pulumi config validation rejects placeholder `REQUIRED_OVERRIDE`; Makefile preflight aborts before `pulumi up` if CIDR is wide-open | Block stays narrow even if config.py is bypassed |
| P1-05 | Critical theme #4 (ansible host key) | `ansible/ansible.cfg` | Ansible CI (if any) on test inventory | **BOTH** `host_key_checking` and `ssh_args` must be fixed |
| P1-06 | Critical theme #4 (systemd hardening) | `ansible/roles/relay-xdp/templates/relay-xdp.service.j2` | Boot relay-xdp under hardened unit in compose / staging; verify XDP still attaches | Add `NoNewPrivileges`, `ProtectSystem=strict`, `ProtectHome=yes`, `PrivateTmp=yes`, `RestrictAddressFamilies=AF_INET AF_INET6 AF_NETLINK`. Capability bounding already correct |
...
| P1-08 | Cross-cutting (parity drift, was Phase 2) | `tests/fixtures/pittle_chonkle_vectors.json`; consumers in `relay-xdp-ebpf` (build-time const), `relay-xdp/src/packet_filter.rs`, `relay-sdk/src/route/mod.rs` | New cross-crate test that loads the fixture in all three contexts | Catches the "single most likely correctness regression" v1 itself called out |
| P1-09 | Cross-cutting (wire layout, was Phase 2) | `relay-xdp-common/src/lib.rs` | `cargo build` on workspace fails if size changes | `const _: () = assert!(core::mem::size_of::<RelayConfig>() == 88);` etc. for every wire struct |
...
| P1-14 | High - Backend HTTP route auth surface (**CONFIRMED HIGH** by v2 SG verification) | `relay-backend/src/main.rs:89` (bind), `handlers.rs::create_router` (route group split), possibly `infra/network.py` (SG narrowing if internal port is added) | New compose test asserting `/cost_matrix` returns 401/403 from non-internal client; `/relay_update` continues to work over the public port | Two-server split is the cleanest fix: public port 8090 carries only `/relay_update` + health; internal port (e.g. 8091) bound to the VPC IP carries everything else. If the split is too invasive, fall back to bearer-token middleware on the non-public group |
