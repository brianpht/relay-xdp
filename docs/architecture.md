...
Pure Rust SDK for game clients and servers that connect to the relay-xdp network.
Wire-compatible with `relay-xdp` (XDP relay node) and `relay-xdp-common` (shared types).
Compiles to `rlib`, `cdylib`, and `staticlib` for use from Rust, C, or any FFI-capable language.
...
- **DDoS filter parity**: `route/mod` pittle/chonkle output must match `packet_filter.rs` in relay-xdp
  and the eBPF filter - change one, change all three
...
> **Detailed architecture**: see [`relay-sdk/ARCHITECTURE.md`](../relay-sdk/ARCHITECTURE.md)
> for module dependency diagram, FFI contract, wire compat test vectors, and benchmark results.
