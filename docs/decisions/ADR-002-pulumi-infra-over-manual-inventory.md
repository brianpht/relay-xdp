...
- **Negative:** Operators need Pulumi CLI installed and S3 backend credentials
  configured. `pulumi up` is an additional step before Ansible. Staging EIPs incur
  $0.005/hr cost when instances are stopped (mitigated by `pulumi destroy --stack
  staging` when idle).
- **Neutral:** `ansible/inventory/production.yml` and `staging.yml` become generated
  files. They should not be edited manually - source of truth is Pulumi state.
...
5. `pulumi preview --stack staging` - verify 25 resources, 0 errors.
