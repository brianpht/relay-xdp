...
This is a complete Rust rewrite of a C-based XDP relay. The only remaining C is a small kernel module that exposes SHA-256 and XChaCha20-Poly1305 as eBPF kfuncs.
...
Wire compatibility tests verify that every `#[repr(C)]` struct matches the original C layout byte-for-byte. This ensures the Rust eBPF program is drop-in compatible with existing relay infrastructure.
