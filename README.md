...
A high-performance UDP game relay that processes packets at the NIC driver level using Linux XDP (eXpress Data Path), written in Rust + eBPF + C.

## Overview

`relay-xdp` routes UDP game traffic between clients, servers, and other relays with sub-microsecond per-packet latency. The eBPF data plane runs inside the kernel at the NIC driver level, forwarding, reflecting, or dropping packets **before** the kernel even allocates a socket buffer. A userspace control plane handles relay registration, health reporting, and session management at 1 Hz.

This is a complete Rust + eBPF implementation. The only C code is a small kernel module that exposes SHA-256 and XChaCha20-Poly1305 as eBPF kfuncs.
...
- **DDoS Filtering**: FNV-1a based pittle/chonkle filter rejects garbage packets in nanoseconds, before any map lookup or crypto
- **200K Concurrent Sessions**: LRU hash maps with O(1) lookup, kernel-managed eviction
- **Zero-Copy Pipeline**: Packets are never copied between kernel and userspace. eBPF operates directly on the NIC's DMA ring buffer
...
- **14 Packet Types**: Route request/response, client-to-server/server-to-client, session ping/pong, continue request/response, client/server/relay ping/pong
...
`relay-backend` is the route computation brain for the entire relay network. It runs as a separate async binary (tokio + axum) and:
...
- **Optimize2** - Multi-threaded route finder using `std::thread::scope`, tries 5 route patterns (direct + 1-3 intermediates), keeps top 16 routes per pair
- **Redis Leader Election** - Multiple instances run concurrently; leader writes to Redis, all instances serve consistent data

For detailed architecture, wire format specs, and the full relay-xdp interaction protocol, see [relay-backend/ARCHITECTURE.md](relay-backend/ARCHITECTURE.md).
...
The cheapest rejection always comes first. DDoS garbage is dropped before touching any BPF map or crypto. Most valid packets never leave the NIC.
...
For detailed performance design principles and optimization guidelines, see [docs/performance_design.md](docs/performance_design.md).
...
Wire compatibility tests verify that every `#[repr(C)]` struct has identical layout across userspace and eBPF targets. This ensures the Rust eBPF program is drop-in compatible with existing relay infrastructure.
...
- [Relay Backend Architecture](relay-backend/ARCHITECTURE.md): route optimization, wire format, encoding, relay-xdp interaction protocol
